...
| `getTriageQueue()`                                                | Tickets not yet approved — `Not Started`/`Reviewing`/`Reviewed`/`Declined` — ordered oldest first; the human reviewer's working set. Explicitly **not** `cacheable` — see [Key Design Decisions](#key-design-decisions)                                                                                                                                                                                                                                                             |
...
Queue of tickets awaiting triage or re-review. Loads `getTriageQueue()` **imperatively** (not via `@wire`) — see [Key Design Decisions](#key-design-decisions) for why. `ticketsForDisplay` getter enriches each record with a status-coloured CSS class. Cards show name, triage status, type, and priority.

**Live refresh, not tied to remounting:**

- Subscribes to the `TicketTriageChannel` Lightning Message Channel with `APPLICATION_SCOPE` and reloads on receipt — picks up tickets created from `ticketSubmit` in the Utility Bar, a different page region than this component's App Page body, while the Triage view is already open.
- Wired to `CurrentPageReference` and reloads on every emission — this app has tab persistence enabled (`ProjectManagement.app-meta.xml`, `isNavTabPersistenceDisabled = false`), so navigating to a ticket's record page and back does not reliably destroy and recreate this component. `CurrentPageReference` re-emits on every navigation event regardless, so a deleted ticket disappears from the queue on return without a manual browser refresh.
